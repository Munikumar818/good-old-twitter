...
export default function EditProfile({user,LoggedInUser}){
  ...
  const loggedInUser={LoggedInUser}
    ...
    async function HandleSave(){
        ...
        if(editedInfo){
            await axios.patch(`https://twitter-backend-1.onrender.com/userUpdates/${user?.email}`,editedInfo);
    ...
    return(
        ...
        <form className="fill-content">
            <TextField className="text-field" fullWidth label='Name' id='fullWidth' variant="filled" onChange={(e)=>setName(e.target.value)}  
            defaultValue={loggedInUser[0]?.name ? loggedInUser[0]?.name : ''} />
            
            <TextField className="text-field" fullWidth label='Bio' id='fullWidth' variant="filled" onChange={(e)=>setBio(e.target.value)}  
            defaultValue={loggedInUser[0]?.bio ? loggedInUser[0]?.bio : ''} />
            
            <TextField className="text-field" fullWidth label='Location' id='fullWidth' variant="filled" onChange={(e)=>setLocation(e.target.value)}  
            defaultValue={loggedInUser[0]?.location ? loggedInUser[0]?.location : ''} />
            
            <TextField className="text-field" fullWidth label='Website' id='fullWidth' variant="filled" onChange={(e)=>setWebsite(e.target.value)}  
